          ...
          <div className="mb-4">
            <div className="flex items-baseline justify-between mb-2">
              <span className="text-3xl font-bold">
                ${(data.totalSpent / 1000000).toFixed(2)}M
              </span>
              <span className="text-sm text-muted-foreground">
                / ${(data.totalAllocated / 1000000).toFixed(2)}M
              </span>
            </div>
            <Progress 
              ...
              className="h-3"
            />
            <p className={`text-sm font-semibold mt-2 ${getTextColor(data.overallPercentage)}`}>
              {data.overallPercentage}% Utilized
            </p>
          </div>
          ...
          <div className="space-y-2 pt-4 border-t">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold">
                ${((data.totalAllocated - data.totalSpent) / 1000).toFixed(0)}K
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Budget Status</span>
              <span className={`font-semibold ${getTextColor(data.overallPercentage)}`}>
                {data.overallPercentage < 70 ? "Healthy" : data.overallPercentage < 90 ? "Moderate" : "High"}
              </span>
            </div>
          </div>
        </CardContent>
