...
import { Module, SubModule, UserType, PermissionAction } from "@/types/roleManagement";
...
const getActionsForPath = (path: string): PermissionAction[] => {
  ...
  // Create/Add paths - write focused
  if (path.includes('create') || path.includes('add')) {
    return ['write'];
  }
  
  // Approval paths - read and edit
  ...
  // Management modules - full access
  if (path.includes('requirements') || path.includes('quotations') || 
      path.includes('purchase-orders') || path.includes('orders') ||
      path.includes('projects') || path.includes('rfqs')) {
    return ['read', 'write', 'edit', 'delete', 'download'];
  }
  
  // Team and stakeholder management
  if (path.includes('team') || path.includes('stakeholders') || 
      path.includes('members') || path.includes('role-management')) {
    return ['read', 'write', 'edit', 'delete'];
  }
  
  // Catalog and inventory
  if (path.includes('catalog') || path.includes('inventory') || 
      path.includes('fleet') || path.includes('services')) {
    return ['read', 'write', 'edit', 'delete', 'download'];
  }
...
const getCategoryForPath = (path: string): string => {
  ...
  if (path.includes('requirements') || path.includes('projects') || path.includes('workflow')) return 'Project Management';
  if (path.includes('quotations') || path.includes('purchase-orders') || path.includes('orders')) return 'Procurement';
  if (path.includes('messages') || path.includes('notifications')) return 'Communication';
  if (path.includes('team') || path.includes('stakeholders') || path.includes('role')) return 'Administration';
  if (path.includes('analytics') || path.includes('reports')) return 'Analytics';
  if (path.includes('settings') || path.includes('profile')) return 'Configuration';
  if (path.includes('catalog') || path.includes('products')) return 'Product Management';
  if (path.includes('services') || path.includes('rfqs')) return 'Service Management';
  if (path.includes('logistics') || path.includes('fleet') || path.includes('deliveries')) return 'Logistics Operations';
...
export const canManageUserType = (managerType: UserType, targetType: UserType): boolean => {
  // IndustryAdmin can manage all types
  if (managerType === 'IndustryAdmin') {
    return true;
  }
  
  // Vendors can only manage their own type
  return managerType === targetType;
};